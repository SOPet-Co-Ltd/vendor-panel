...
export const ProductOrganizationForm = ({ product }: ProductOrganizationFormProps) => {
  ...
  const form = useExtendableForm({
    defaultValues: {
      ...
      custom_tag_1: (product.custom_tags || []).find(ct => ct.type === 'pet_type')?.id || '',
      custom_tag_2: (product.custom_tags || []).find(ct => ct.type === 'brand')?.id || ''
    },
  ...
  const handleSubmit = form.handleSubmit(async data => {
    const selectedTagIds = [data.custom_tag_1, data.custom_tag_2].filter((v): v is string => !!v);
