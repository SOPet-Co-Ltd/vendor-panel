import { CreatePromotionRuleDTO, HttpTypes, PromotionRuleDTO } from '@medusajs/types';
  ...
  const handleSubmit = (
    ...
    return async function (data: EditRulesType) {
      ...
      for (const rule of disguisedRules) {
        const value = Array.isArray(rule.values) ? rule.values[0] || null : rule.values || null;
      ...
      if (rulesToCreate.length > 0) {
        ...
        if (rulesToRemove?.length) {
          const removeIds = rulesToRemove.map(r => r.id).filter((id): id is string => Boolean(id));
          await removePromotionRules({
            rules: removeIds
          });
        }
